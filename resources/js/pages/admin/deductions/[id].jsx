...
const show = (props) => {
	...
	useEffect(() => {
		// Set page
		props.setPage({ name: "View Deduction", path: ["deductions", "view"] })

		Axios.get(`api/deductions/${id}`)
			.then((res) => {
				setDeduction(res.data.data)
			})
			.catch((err) => props.errors(["Failed to Fetch Deduction"]))
	}, [])
				...
				<div className="offset-xl-2 col-xl-8 col-lg-12 col-md-12 col-sm-12 col-12">
					<div className="card p-5">
						<div className="card-header bg-white border-0 d-flex justify-content-between">
							...
							<div>
								<h2 className="mb-0">DEDUCTION MEMO</h2>
							</div>
						</div>
						<div className="card-body">
							<div className="d-flex justify-content-between mb-4">
								<div className="">
									...
									<div className="text-muted">Tenant: {deduction.tenantName}</div>
									<div className="text-muted">Unit: {deduction.unitName}</div>
									<div className="text-muted">Phone: {deduction.tenantPhone}</div>
									<div className="text-muted">Email: {deduction.tenantEmail}</div>
								</div>
								<div className="text-end">
									<h5 className="">Deduction Memo No: {deduction.number}</h5>
									<div className="text-muted">Date: {deduction.createdAt}</div>
								</div>
							</div>
								...
								<table className="table table-borderless bg-white">
									...
									<tbody>
										<tr>
											...
											<td className="text-capitalize">{deduction.description}</td>
										...
										<tr className="border-bottom border-top">
											<td colSpan={deduction.type == "water" ? 3 : 0}></td>
